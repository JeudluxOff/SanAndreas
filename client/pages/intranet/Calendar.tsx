...
import { Calendar as CalendarIcon, Clock, MapPin, Users, ChevronLeft, ChevronRight, Plus, MoveVertical as MoreVertical, Shield, Bell, CircleCheck as CheckCircle2, ShieldAlert, CalendarDays } from "lucide-react";
...
const SERVICE_OPTIONS = ['CABINET', 'SECURITE_PUBLIQUE', 'JUSTICE', 'SANTE_HUMAINS', 'TRESOR_COMMERCE', 'COMMUNICATION', 'ADMINISTRATION_GENERALE'];
const TYPE_OPTIONS = ['reunion', 'briefing', 'officiel', 'critique', 'securite', 'economie', 'justice'];

export default function Calendar() {
  const { user, hasPermission, emergencyMode, toggleEmergencyMode, logAction } = useAuth();
  ...
  const [newEvent, setNewEvent] = useState({
    ...
    participants: '',
    type: 'reunion',
    service: user?.service_id || 'CABINET',
  });

  const rawEvents = store.getCalendarEvents();
  ...
  const handleCreateEvent = () => {
    ...
    const event = {
      ...
      location: newEvent.location.trim() || 'Salle de conférence',
      participants: parseInt(newEvent.participants) || 1,
      type: newEvent.type,
      service: newEvent.service,
      date: selectedDate.toISOString(),
    };
    store.addCalendarEvent(event);
    logAction('Création réunion', { title: event.title });
    setShowCreateModal(false);
    setNewEvent({ title: '', time: '', location: '', participants: '', type: 'reunion', service: user?.service_id || 'CABINET' });
  };
          ...
          <div className="flex items-center gap-2">
            {hasPermission('planning:create') && (
              <Button
                ...
                className="bg-primary hover:bg-primary/90 text-white font-bold uppercase tracking-widest text-[10px] gap-2 px-6 h-11"
              >
              ...
              <CardContent className="p-8 pt-10">
                <div className="grid grid-cols-7 gap-y-6 text-center mb-8">
                  ...
                  {Array.from({ length: 31 }).map((_, i) => {
                    ...
                    const hasEvents = EVENTS.some(e => isSameDay(e.date, addDays(startOfMonth(selectedDate), i)));
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 overflow-hidden relative">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="icon" onClick={() => scroll('left')} className="h-8 w-8 text-slate-400 hover:text-primary">
                    <ChevronLeft className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => scroll('right')} className="h-8 w-8 text-slate-400 hover:text-primary">
              ...
              >
                {scrollDates.map((date, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className={cn("text-[10px] font-black uppercase tracking-widest mb-1 opacity-70", isSelected && "opacity-100")}>
                    ...
                    <div className="space-y-6">
                      {dayEvents.length > 0 ? dayEvents.map((event) => (
                                  ...
                                  <div className="flex flex-col items-end gap-4">
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent align="end" className="font-bold text-xs uppercase tracking-widest">
                                        <DropdownMenuItem onClick={() => logAction('Modifier réunion', { id: event.id })}>
                                          Modifier
                                        </DropdownMenuItem>
                                        <DropdownMenuItem onClick={() => logAction('Annuler réunion', { id: event.id })} className="text-red-600">
                                          Annuler la réunion
                                        </DropdownMenuItem>
                                      </DropdownMenuContent>
                                    </DropdownMenu>
                        ...
                        <div className="flex items-center gap-6 pl-[52px] opacity-40">
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Aucun événement à cette date</p>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            <DialogTitle className="uppercase font-black tracking-tight flex items-center gap-2">
              <CalendarIcon className="w-4 h-4 text-primary" /> Nouvelle Réunion
            </DialogTitle>
            <DialogDescription>Planifiez une nouvelle réunion pour le {format(selectedDate, 'EEEE d MMMM', { locale: fr })}.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Intitulé *</label>
              <Input
                placeholder="Ex: Réunion de coordination sécurité"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Horaire *</label>
                <Input
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Participants</label>
                <Input
                  type="number"
                  min="1"
                  placeholder="5"
                  value={newEvent.participants}
                  onChange={e => setNewEvent(f => ({ ...f, participants: e.target.value }))}
                />
              </div>
            </div>
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Lieu</label>
              <Input
                placeholder="Ex: Salle 4B — Cabinet"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Service</label>
                <select
                  className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm font-medium focus:ring-primary focus:border-primary outline-none bg-white"
                  value={newEvent.service}
                  onChange={e => setNewEvent(f => ({ ...f, service: e.target.value }))}
                >
                  {SERVICE_OPTIONS.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
                </select>
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Type</label>
                <select
                  className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm font-medium focus:ring-primary focus:border-primary outline-none bg-white"
                  value={newEvent.type}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateEvent} disabled={!newEvent.title.trim() || !newEvent.time.trim()}>
